...
ramas_turismo_6d <- openxlsx::read.xlsx("Entradas/ramas turismo 6 D.xlsx") %>% 
  filter(!COD_ACTIVIDAD_F883 %in% c(473000,681098,681099,780000,562091)) %>% 
  pull(COD_ACTIVIDAD_F883)

clae_diccionario <- read.csv("Entradas/clae_diccionario.csv") %>% select(clae6,clae6_desc)
```
...
El presente documento expone información de puestos de trabajo registrados en ramas turísticas, participación por género y empleadores, a partir de datos mensuales para el período 2007-2021. Se podrá seguir periódicamente estos indicadores [aquí](https://tableros.yvera.tur.ar/reportes/informe_empleo.html), con el agregado de gráficos interactivos.
...
A continuación se presenta la evolución de la tasa de feminización de los puestos trabajo en todas las ramas turísticas, agrupadas por categorías. Cada línea gris es una rama a 6 dígitos y el **grosor** de cada una depende del peso que tiene el empleo promedio de todo el período en esa rama sobre el total.
...
La información del MDP incluye, además, a las empresas que declararon trabajadores en el mes de referencia, agrupadas también en ramas a 6 dígitos. A continuación se presenta, entonces, la evolución de este indicador en ramas turísticas. 
...
[**Reporte**](https://tableros.yvera.tur.ar/informe_empleo.html)
